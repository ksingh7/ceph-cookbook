...
##################################### Configuration for ceph-node1 #####################################################
				...
				v.gui = true
                                ...
                                v.customize ['createhd', '--filename', ceph_node1_disk2, '--variant', 'Fixed', '--size', 1 * 1024]
                                ...
                                v.customize ['createhd', '--filename', ceph_node1_disk3, '--variant', 'Fixed', '--size', 1 * 1024]
                                ...
                                v.customize ['createhd', '--filename', ceph_node1_disk4, '--variant', 'Fixed', '--size', 1 * 1024]
		 ...
		 config.vm.define :"ceph-node2" do |node2|
			...
			node2.vm.provider "virtualbox" do |v|
				...
				unless File.exist?(ceph_node2_disk2)
                        	v.customize ['createhd', '--filename', ceph_node2_disk2, '--variant', 'Fixed', '--size', 1 * 1024]
				...
				unless File.exist?(ceph_node2_disk3)
                        	v.customize ['createhd', '--filename', ceph_node2_disk3, '--variant', 'Fixed', '--size', 1 * 1024]
                		v.customize ['storageattach', :id,  '--storagectl', 'SATA', '--port', 2, '--device', 0, '--type', 'hdd', '--medium', ceph_node2_disk3]
                        	...
                        	v.customize ['createhd', '--filename', ceph_node2_disk4, '--variant', 'Fixed', '--size', 1 * 1024]
		 ...
		 config.vm.define :"ceph-node3" do |node3|
			...
			node3.vm.provider "virtualbox" do |v|
				...
				unless File.exist?(ceph_node3_disk2)
                        	v.customize ['createhd', '--filename', ceph_node3_disk2, '--variant', 'Fixed', '--size', 1 * 1024]
				...
				unless File.exist?(ceph_node3_disk3)
                        	v.customize ['createhd', '--filename', ceph_node3_disk3, '--variant', 'Fixed', '--size', 1 * 1024]
                		v.customize ['storageattach', :id,  '--storagectl', 'SATA', '--port', 2, '--device', 0, '--type', 'hdd', '--medium', ceph_node3_disk3]
                        	...
                        	v.customize ['createhd', '--filename', ceph_node3_disk4, '--variant', 'Fixed', '--size', 1 * 1024]
...
##################################### Configuration for ceph-node4 #####################################################
		 ...
		 config.vm.define :"centos7-standard" do |centos7|
        		centos7.vm.box = BOX
        		centos7.vm.box_url = BOX_URL
			centos7.vm.provider "virtualbox" do |v|

          			v.customize ["modifyvm", :id, "--memory", "1024"]
           			v.name = "centos7-standard" 
				v.gui = true

        		centos7.vm.hostname = "vagrant" 
